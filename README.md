...
当前页面提供项目、机房、机柜、设备和接口的基础维护，支持按机柜数量和位置自动调整的机房二维平面图、网格背景、机柜拖动、网格吸附、缩放、平移、自动适配和布局保存；在设备面板中可以从实际接口坐标查看端口到端口的连接关系，拖拽端口创建连接，或从接口选择其他机柜中的目标接口，并维护连接属性。支持接口批量生成、全局搜索、JSON 导入/导出以及当前视图 SVG/PNG 导出。
